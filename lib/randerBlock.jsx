  ...
  switch (type) {
      ...
      return (
        <details>
          ...
          {value.children?.map((block) => (
            <div className="pl-4 text-md leading-none" key={block.id}>
              {renderBlock(block)}
            </div>
          ))}
        </details>
    ...
    case "video":
      ...
      return (
        <figure className="my-8">
          <iframe
            className="max-w-2xl w-full  sm:h-96 h-52 "
            src={videoSrc}
            allowFullScreen
          ></iframe>
          {Vcaption && (
            <figcaption className="text-gray-500">{Vcaption}</figcaption>
          )}
        </figure>
      );
    case "callout":
      ...
      return (
        <div className={` p-2 my-4 rounded-sm flex gap-4  relative`}>
          <div
            style={{ backgroundColor: calloutColor, opacity: "0.1" }}
    ...
    case "quote":
      return (
        <blockquote
          className="border-l-4 border-stone-600 pl-6 my-8 text-xl text-stone-500 italic"
          key={id}
        >
    ...
    return (
      <span
        ...
        className={`${bold ? "font-bold" : ""} ${
          code
            ? "bg-code-bg dark:text-dark1 font-mono text-sm  py-1 px-1.5 rounded-md"
            : ""
