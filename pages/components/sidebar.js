...
const currentTheme = theme === "system" ? systemTheme : theme;
    if(currentTheme === "dark"){
     return(
  el ==="icon" ? <div onClick={() => setTheme("light")} className=" cursor-pointer w-10 h-10 flex justify-center items-center text-2xl hover:bg-stone-100 dark:hover:bg-neutral-800 active:bg-stone-200 dark:active:bg-neutral-600 rounded-full text-stone-500 dark:text-neutral-100 "> <HiOutlineSun /></div> : 
  <Image className="w-8 h-8 rounded-full" src={myLogo} alt="myLogo" />
     )
   }else {
      return(
       el ==="icon" ? <div onClick={() => setTheme("dark")} className="cursor-pointer w-10 h-10 flex justify-center items-center text-2xl hover:bg-stone-100 dark:hover:bg-neutral-800 active:bg-stone-200 dark:active:bg-neutral-600 rounded-full text-stone-500 dark:text-neutral-100 "> <HiOutlineMoon /></div> : <Image className="  w-8 h-8 rounded-full" src={myLogoDark} alt="myLogo" />
      )
}
  }
...
// disableBodyScroll(document);
  ...
  return (
    ...
    <div className=" w-full bg-white lg:dark:bg-transparent dark:bg-dark3 lg:w-20 lg:border-r border-gray-200 dark:border-neutral-700 lg:py-10 box-border lg:flex flex-row z-10  lg:flex-col justify-between items-center h-16 lg:h-screen fixed lg:sticky bottom-0 lg:top-0 left-0">
      <Link className="hidden lg:inline  lg:hover:bg-stone-100 dark:hover:bg-neutral-800 lg:active:bg-stone-200 dark:active:bg-neutral-600 rounded-full text-stone-500  dark:text-neutral-100" href="/">
     {changeTheme()}
        </Link>
